...
conditional_DEA_DDF <- function(
    X, Y, exogenous = FALSE, alpha = FALSE,
    m, B, RTS = "crs", ORIENTATION = "in", similarity = FALSE,
    inclusion = FALSE, print = FALSE) {

  # X <- as.data.frame(input)
  # Y <- as.data.frame(output)
  ...
  for (i in 1:n) {
    ...
    for (l in 1:k) {
      cond_output[, l] <- Y_Rob[, l] >= as.numeric(y[l])
    }

    cond_input <- matrix(TRUE, nrow = nrow(X_Rob), ncol = s)
    for (l in 1:s) {
      cond_input[, l] <- X_Rob[, l] <= as.numeric(x[l])
    }
