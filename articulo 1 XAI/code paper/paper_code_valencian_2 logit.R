...
label_efficiency$data_proportions$efficient
...
result_names <- c(overall_names, byClass_names) 
...
valid_data$class_efficiency <- factor(
...
for(balance_i in balance) {
  ...
  # train model
  # toma el data.frame correspondiente al balance_i actual
  df <- train_data_SMOTE[[as.character(balance_i)]]

  # mapear: not_efficient -> 0, efficient -> 1, y dejarlo como factor con 0 como primer nivel
  df$class_efficiency <- ifelse(df$class_efficiency == "efficient", 1, 0)
  df$class_efficiency <- factor(
    df$class_efficiency,
    levels = c(0,1)
  )

  train_data_SMOTE[[as.character(balance_i)]] <- df
  ...
  model_step <- stats::step(model_logit, direction = "both", trace = TRUE)
  model_logit <- model_step
  summary(model_logit)
  
  save_models[[as.character(balance_i)]] <- model_logit
    
  # y_hat <- predict(best_model_nnet, valid_data)
  y_hat <- predict(model_logit, valid_data)
...
tied_imbalance <- df_hyperparams %>%
  filter(dplyr::near(Balanced_Accuracy, max(Balanced_Accuracy))) %>%   # empates en la métrica principal
  pull(imbalance) %>%
  unique() %>%
  sort()
...
train_data <- train_data_SMOTE[[as.character(tied_imbalance)]]
...
if (method_imp == "SA") {
  ...
  f_pred <- function(object, newdata) {
    pr <- as.data.frame(predict(object, newdata = newdata, type = "response"))
...
# # mean vector 
# importance <- colMeans(data[, c(new_x, new_y)])
...
# y_hat <- predict(best_model_nnet, valid_data)
y_hat <- predict(save_models[[as.character(tied_imbalance)]], valid_data)
y_hat <- ifelse(y_hat > 0.5, 1, 0)
y_hat <- factor(
  y_hat,
  levels = c(0, 1),
...
# save(final_model, file = "result_logit_g_x0_y0.Rdata")
