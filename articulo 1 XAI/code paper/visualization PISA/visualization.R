...
load("C:/Users/Ricardo/OneDrive - UMH/Documentos/Cafee/resultados_art_XAI_def.RData")
...
# svm
imp_svm <- information_region$ML_models$svmPoly$importance

data <- information_region$ML_models$svmPoly$importance[4,4:ncol(data)]
...
plot <- ggplot(data = df_long, aes(x = reorder(variable, importance), y = importance)) +
  ...
  labs(x = "variables",
       ...
       title = "Bar plot with the 1D input importances for PISA 2018") +
...
plot 
ggsave(plot = plot, dpi = 600, filename = "1D bar plot.png")
